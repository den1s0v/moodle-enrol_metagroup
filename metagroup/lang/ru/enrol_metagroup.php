...
$string['addgroupsuffix'] = 'Добавлять суффикс «(связ.)» к именам групп';
$string['addgroupsuffix_desc'] = 'Если включено, автоматически создаваемые целевые группы получают суффикс в имени (например, «Группа (связ.)»). Нумерация (2), (3) при совпадении имён сохраняется и при отключённой настройке.';
...
$string['creategroup_one'] = 'Создать общую группу с именем курса';
...
$string['linkedgroup_help'] = 'Связываемая группа из выбранного курса. Цепочки связей поддерживаются — можно использовать группу из промежуточного курса, который получает участников через метагруппу (например, A→B→C). Рекурсивные связи блокируются при валидации.';
...
$string['sourcecourseschain'] = 'Цепочка курсов-источников';
$string['recalculate_links'] = 'Актуализировать связи';
$string['recursive_link_error'] = 'Невозможно создать связь: целевой курс входит в цепочку источников (рекурсивная связь).';
